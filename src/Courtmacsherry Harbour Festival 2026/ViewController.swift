...
class ViewController: UIViewController, WKNavigationDelegate, UIDocumentInteractionControllerDelegate {
    ...
    override func viewDidLayoutSubviews() {
        ...
        CourtmacsherryHarbourFestival2026.webView.frame = calcWebviewFrame(webviewView: webviewView, toolbarView: nil)
    }
    
    @objc func keyboardWillHide(_ notification: NSNotification) {
        CourtmacsherryHarbourFestival2026.webView.setNeedsLayout()
    }
    
    func initWebView() {
        CourtmacsherryHarbourFestival2026.webView = createWebView(container: webviewView, WKSMH: self, WKND: self, NSO: self, VC: self)
        webviewView.addSubview(CourtmacsherryHarbourFestival2026.webView);
        
        CourtmacsherryHarbourFestival2026.webView.uiDelegate = self;
        
        CourtmacsherryHarbourFestival2026.webView.addObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress), options: .new, context: nil)

        if(pullToRefresh){
            let refreshControl = UIRefreshControl()
            refreshControl.addTarget(self, action: #selector(refreshWebView(_:)), for: UIControl.Event.valueChanged)
            CourtmacsherryHarbourFestival2026.webView.scrollView.addSubview(refreshControl)
            CourtmacsherryHarbourFestival2026.webView.scrollView.bounces = true
        }

        if #available(iOS 15.0, *), adaptiveUIStyle {
            themeObservation = CourtmacsherryHarbourFestival2026.webView.observe(\.themeColor) { [unowned self] webView, _ in
                let backgroundColor = CourtmacsherryHarbourFestival2026.webView.underPageBackgroundColor;
                let themeColor = CourtmacsherryHarbourFestival2026.webView.themeColor;
    ...
    @objc func refreshWebView(_ sender: UIRefreshControl) {
        CourtmacsherryHarbourFestival2026.webView?.reload()
...
//        toolbarView.autoresizingMask = [.flexibleTopMargin, .flexibleRightMargin, .flexibleWidth]
    ...
    func overrideUIStyle(toDefault: Bool = false) {
        if #available(iOS 15.0, *), adaptiveUIStyle {
            if (((htmlIsLoaded && !CourtmacsherryHarbourFestival2026.webView.isHidden) || toDefault) && self.currentWebViewTheme != .unspecified) {
                UIApplication
                    .shared
                    .connectedScenes
    ...
    @objc func loadRootUrl(cachePolicy: NSURLRequest.CachePolicy = .useProtocolCachePolicy) {
        CourtmacsherryHarbourFestival2026.webView.load(URLRequest(url: SceneDelegate.universalLinkToLaunch ?? SceneDelegate.shortcutLinkToLaunch ?? rootUrl, cachePolicy: cachePolicy))
    }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!){
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
            CourtmacsherryHarbourFestival2026.webView.isHidden = false
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if (keyPath == #keyPath(WKWebView.estimatedProgress) &&
                CourtmacsherryHarbourFestival2026.webView.isLoading &&
                !self.loadingView.isHidden &&
                !self.htmlIsLoaded) {
                    var progress = Float(CourtmacsherryHarbourFestival2026.webView.estimatedProgress);
    ...
    deinit {
        CourtmacsherryHarbourFestival2026.webView.removeObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress))
    }
}
...
extension ViewController: WKScriptMessageHandler {
  func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if message.name == "print" {
            printView(webView: CourtmacsherryHarbourFestival2026.webView)
        }
